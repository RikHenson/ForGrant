function f=plot_parameters(Ep,Cp,gP,xlabels)
    ...
    %legend([h{1}(1) h{2}(1)], {'Prior','Posterior'})
    text(max([x(:); xp(:)]), max([gP(:); P(:)]),'Prior','Color',[0.5 0.5 0.5],'FontSize',24)
    text(max([x(:); xp(:)]), 0.9*max([gP(:); P(:)]),'Posterior','Color',[51 153 255]./255,'FontSize',24)
